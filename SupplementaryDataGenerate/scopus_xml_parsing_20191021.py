...
def getUniqueAuthorcountry(sourcedict):
    author_group = getTargetitem(sourcedict, ['xocs:doc', 'xocs:item', 'item', 'bibrecord', 'head', 'author-group'])
    auct = []
    if(not isinstance(author_group, list)):
        if(author_group == "ERR"):
...
if(__name__ == "__main__"):
    xmlbuffer = ""
    count = 0
    print("\t".join(["#eid", "pubyear", "asjc", "srcid", "doctype", "srctype", "authorcountry"]))
    for line in sys.stdin:
        ...
        if(line.find("</xocs:doc") >= 0):
            ...
            AuthorCountry = getUniqueAuthorcountry(tempdict)
            #print("Parsed")
            print(EID, PubYear, ASJC, SrcID, DocType, SrcType, AuthorCountry, sep="\t")
            xmlbuffer = ""
